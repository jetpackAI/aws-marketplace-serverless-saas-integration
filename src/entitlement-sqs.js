  ...
  await Promise.all(
    event.Records.map(async (record) => {
      ...
      if (message.action === "entitlement-updated") {
        ...
        const entitlementsResponse = await marketplaceEntitlementService
          .getEntitlements(entitlementParams)
          .promise();

        console.log("entitlementsResponse", entitlementsResponse);

        const isExpired =
          new Date(entitlementsResponse.Entitlements[0].ExpirationDate) <
          new Date();
