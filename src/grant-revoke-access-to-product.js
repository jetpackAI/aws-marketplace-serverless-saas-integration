...
exports.dynamodbStreamHandler = async (event) => {
  await Promise.all(
    event.Records.map(async (record) => {
      ...
      console.log(
        `DynamoDb record updated! OldImage: ${JSON.stringify(
          oldImage
    ...
    */
      ...
      const revokeAccess =
        newImage.subscription_expired === true &&
      ...
      if (grantAccess || revokeAccess || entitlementUpdated) {
        ...
        if (grantAccess) {
          ...
          message = `Grant access to new SaaS customer: ${JSON.stringify(
            newImage
          )}`;
        } else if (revokeAccess) {
          subject = "AWS Marketplace customer end of subscription";
          message = `Revoke access to SaaS customer: ${JSON.stringify(
            newImage
          )}`;
